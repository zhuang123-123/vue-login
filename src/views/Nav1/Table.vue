<template>
    <section>
        <!-- 工具条 -->
        <el-col :span="24" class="toolbar" style="background: #f2f2f2; padding: 10px 10px 0px 10px; margin: 10px 0px; text-align: left;">
...
export default {
    ...
    methods: {
        // 性别显示转换
        formatSex: function (row, columns) {
            console.log(columns)
        ...
        addSubmit: function () {
            this.$refs.addForm.validate((valid) => {
                if (valid) {
                    this.$confirm('确认提交么?', '提示', {}).then((res) => {
                        console.log(res)
